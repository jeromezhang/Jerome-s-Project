...
//lunar information
public class MonthView extends View implements View.OnCreateContextMenuListener, OnSharedPreferenceChangeListener {
    ...
    private static int LUNAR_INFO_TEXT_SIZE = 18;
    private static int WEEK_INDEX_TEXT_SIZE = 16;

    
    private static int WEEK_INDEX_BAR_WIDTH_LANDSCAPE = 28;
    private static int WEEK_INDEX_BAR_WIDTH_PORTRAIT = 25;
...
//    private boolean mShowWeekNumber = false;//drop google's design
    ...
    private boolean mPreferenceChange = false;
    ...
    public MonthView(MonthActivity activity, Navigator navigator) {
		...
		mShowLunar = prefs.getBoolean(CalendarPreferenceActivity.KEY_SHOW_LUNAR,
				CalendarPreferenceActivity.DEFAULT_VALUE_OF_SHOW_LUNAR);       

        mShowWeek = prefs.getBoolean(CalendarPreferenceActivity.KEY_SHOW_WEEK, 
        		CalendarPreferenceActivity.DEFAULT_VALUE_OF_SHOW_WEEK);
        
        if (mShowLunar) {
        	TextView title = (TextView) activity.findViewById(R.id.title);
            title.setText(Utils.formatMonthYear(getContext(),mViewCalendar) + " " + Utils.lunarMonth(getContext(), mViewCalendar));
        }
       }
    //for lunar information
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        if (key.equals(CalendarPreferenceActivity.KEY_SHOW_LUNAR)) {
            boolean newSetting = sharedPreferences.getBoolean(key, CalendarPreferenceActivity.DEFAULT_VALUE_OF_SHOW_LUNAR);
            if(mShowLunar != newSetting) {
            	mShowLunar = newSetting;
            	// TODO: update UI right now if the view is on top ...
                Log.d("CalendarView e2437c", "  ShowLunar setting changed!!!");
            }
        }

		if (key.equals(CalendarPreferenceActivity.KEY_SHOW_WEEK)) {
			boolean newSetting = sharedPreferences.getBoolean(key,
					CalendarPreferenceActivity.DEFAULT_VALUE_OF_SHOW_WEEK);
			if (mShowWeek != newSetting) {
				mShowWeek = newSetting;
				// TODO: update UI right now if the view is on top ...
				Log.d("CalendarView", "  ShowWeek setting changed!!!");
			}
		}
		mRedrawScreen = true;
		mPreferenceChange = true;
//		invalidate();
    }
    ...
    protected void onDraw(Canvas canvas) {
        if (mRedrawScreen) {
            if (mShowLunar) {
            	TextView title = (TextView) mParentActivity.findViewById(R.id.title);
                title.setText(Utils.formatMonthYear(getContext(),mViewCalendar) + " " + Utils.lunarMonth(getContext(), mViewCalendar));
            }
            else{
            	TextView title = (TextView) mParentActivity.findViewById(R.id.title);
                title.setText(Utils.formatMonthYear(getContext(),mViewCalendar));
            }
            if (mCanvas == null||mPreferenceChange == true) {
    ...
    private void drawGrid(Canvas canvas, Paint p) {
        ...
        int weekIndexBarWidth = 0;
		if (mShowWeek) {
			boolean isLandscape = getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE;
			if (isLandscape)
				weekIndexBarWidth = WEEK_INDEX_BAR_WIDTH_LANDSCAPE ;
			else
				weekIndexBarWidth = WEEK_INDEX_BAR_WIDTH_PORTRAIT;
		}
		int beginX = weekIndexBarWidth;
		int beginColumn = weekIndexBarWidth == 0? 0 : 1;
		int width = getMeasuredWidth() - weekIndexBarWidth;
        final int height = getMeasuredHeight();

        for (int row = 0; row < 6; row++) {
            int y = WEEK_GAP + row * (WEEK_GAP + mCellHeight) - 1;
            canvas.drawLine(0, y, beginX+width, y, p);
        }
        for (int column = 1 - beginColumn; column < 7; column++) {
            int x = mBorder + column * (MONTH_DAY_GAP + mCellWidth) - 1 + beginX;
    ...
    private void drawBox(int day, int weekNum, int row, int column, Canvas canvas, Paint p,
        ...
        int x = mBorder + column*(MONTH_DAY_GAP + mCellWidth) + beginX;
        ...
        // Draw week number
        if (mShowWeek && column == 0) {
            // Draw the banner
            p.setColor(mMonthWeekBannerColor);
            Rect rect = new Rect(r);
            rect.right = rect.left + 1;
            rect.left -= weekIndexBarWidth;
...
/*            p.setStyle(Style.STROKE);
            ...
            int textX = r.left + WEEK_TEXT_PADDING - weekIndexBarWidth;
            int textY;
            textY = (int) (r.top + p.getTextSize() + WEEK_TEXT_PADDING);
            canvas.drawText(String.valueOf(weekNum), textX, textY, p);

            textY = (int) (r.top + p.getTextSize() + WEEK_TEXT_PADDING + WEEK_TEXT_SIZE);
            canvas.drawText(mResources.getString(R.string.week_numbers), textX, textY, p);
        }
		...
		if (mShowLunar) {
			...
			if(isLandscape)
				textY -= 6;//modify for landscape display overlay
			canvas.drawText(String.valueOf(mCursor.getDayAt(row, column)), textX, textY, p);
			p.setStyle(Paint.Style.FILL);
			p.setAntiAlias(true);
			p.setTypeface(null);
			p.setTextSize(LUNAR_INFO_TEXT_SIZE);
			...
			p.setTextAlign(Paint.Align.CENTER);
			int right1 = r.right - BUSY_BITS_WIDTH - BUSY_BITS_MARGIN;
			int textX1 = r.left + (right1 - r.left) / 2; // center of text
			int textY1 = r.bottom - BUSY_BITS_MARGIN - 2; // bottom of text

			Time date = mTempTime;
			date.set(mViewCalendar);

			// date.month += mCursor.getSelectedMonthOffset();
			date.month = mCursor.getMonth();
			date.year = mCursor.getYear();
//			Log.v("monthView", " year before = " + date.year + " month before = " + date.month);
			if (false == mCursor.isWithinCurrentMonth(row, column)) {
				if (0 == row) {
					if (0 == date.month) {
						date.month = 11;
						date.year--;
			...
			date.monthDay = mCursor.getDayAt(row, column);
//			Log.v("monthView", " year after = " + date.year + " month after = " + date.month + " monthday= " + date.monthDay);

			// Restore the saved hour:minute:second offset from when we entered
			// this view.
			// date.second = mSavedTime.second;
			// date.minute = mSavedTime.minute;
			// date.hour = mSavedTime.hour;
			lunarInfo lunarData = Utils.monthlyViewLunarDate(mContext, date);
			if (lunarData.lunarType == lunarInfo.LUNAR_TYPE_FESTIVAL)
				p.setColor(Color.RED);
			else if (lunarData.lunarType == lunarInfo.LUNAR_TYPE_SOLAR_TEAM)
				p.setColor(Color.BLUE);
			canvas.drawText(lunarData.lunarStr, textX1, textY1, p);
		} else {
			// Draw the monthDay number
			p.setStyle(Paint.Style.FILL);
			p.setAntiAlias(true);
    ...
    private void drawingCalc(int width, int height) {
		...
		mCellWidth = (width - weekIndexBarWidth - (6 * MONTH_DAY_GAP)) / 7;
		mBorder = (width - weekIndexBarWidth - 6 * (mCellWidth + MONTH_DAY_GAP) - mCellWidth) / 2;
