...
public class CalendarView extends View
    ...
    private void recalc() {
        ...
        calendar.set(mBaseDate.year, mBaseDate.month, mBaseDate.monthDay);
        Resources r = getResources();
		if (mShowLunar) {
			if (1 == mNumDays)
				mDateRange += " " + Utils.lunarDate(getContext(), mBaseDate);
			else {
				if (mShowWeek)
					mDateRange += " " + Integer.toString(calendar.get(Calendar.WEEK_OF_YEAR))
							+ r.getString(R.string.week_numbers)
							+ " " + Utils.lunarMonth(getContext(), mBaseDate);
				else
					mDateRange += " " + Utils.lunarMonth(getContext(), mBaseDate);
			}
		} else {
			if (7 == mNumDays) {
				if (mShowWeek)
        ...
        //// e2437c
/*        if (1 == mNumDays && mShowLunar)
        {
            Log.d("CalendarView e2437c", "  fetching lunar in recalc()");
            mDateRange += " " + Utils.lunarDate(mContext, mBaseDate);
        }
 */
    ...
    void reloadEvents() {
        ...
        mEventLoader.loadEventsInBackground(mNumDays, events, millis, new Runnable() {
            public void run() {
                ...
                //// e2437c
                // To fix the issue that the lunar date can not be refreshed after the setting changes.
                // It should NOT be here. However, after all since the whole view will be redrawn...
                mTitleTextView.setText(mDateRange);
